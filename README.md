...
行動登録画面から行動登録を行います。関連先企業を入力した後に案件・担当者検索ボタンを押すと関連先企業に紐づいている案件と担当者をセレクトボックスで選択できるようになります。このようにしたことで入力の手間を省くことができます。検索ボタンを押す前は関連先案件、取引先担当者ともにテキストボックスとなっていますが、入力できないようにしています。
